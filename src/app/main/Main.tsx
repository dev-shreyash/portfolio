...
// =========================================
// DATA CONFIGURATION
// Update everything from this single object!
// =========================================
...
export default function Main() {
  ...
  return (
    <>
      ...
      {isMobile ? (
        <MobileBentoHero handleDownload={handleDownload} data={portfolioData} />
      ) : (
        <DesktopHero handleDownload={handleDownload} data={portfolioData} />
      )}
...
// =========================================
const DesktopHero = ({ handleDownload, data }: { handleDownload: () => void; data: typeof portfolioData }) => {
    ...
    <main ref={containerRef} className="relative flex flex-col items-center justify-center w-full min-h-[400px] h-screen text-white bg-transparent z-10">
      <div className="flex flex-col items-center">
        <div className="mb-0 pb-0 text-[12vw] leading-none whitespace-nowrap tracking-tight">
          <span className="desktop-title leading-relaxed">{data.hero.desktopTitle}</span>
        </div>

        <div className="flex flex-col items-center justify-between gap-[2vw]">
          <div className="text-[3vw] fade-up-element">
            <span>{data.hero.roles[0]}</span> | <span>{data.hero.roles[1]}</span>
          </div>

          <div className="flex gap-[1vw] fade-up-element">
            {data.socials.map((social) => (
              <Link key={social.name} href={social.url} target="_blank" className="transition-all duration-300 hover:scale-125">
                <img src={social.icon} alt={social.name} className="w-10 h-10 invert cursor-pointer" />
              </Link>
            ))}
          </div>

          <div className="flex justify-between gap-[1vw] mt-4 fade-up-element">
            <a href={data.hero.resumeLink} onClick={handleDownload} download>
            ...
            <a href={`mailto:${data.hero.email}`} target="_blank">
...
// =========================================
const MobileBentoHero = ({ handleDownload, data }: { handleDownload: () => void; data: typeof portfolioData }) => {
  return (
    <div className="flex flex-col w-full  mx-auto p-4 gap-4 mt-24 font-mono text-sm text-gray-300 relative z-10 ">
      ...
      <div className="flex items-center gap-4 p-5 border border-neutral-800 rounded-xl bg-black/50 backdrop-blur-sm">
        <div className="w-20 h-20 rounded-full overflow-hidden border border-neutral-700 shrink-0">
          <img src={data.hero.profilePhoto} alt={data.hero.firstName} className="w-full h-full object-cover bg-neutral-900 backdrop-grayscale" loading="lazy" width={100} height={100}  />
        </div>
        <div className="flex flex-col justify-center">
          <p className="text-xs text-neutral-500 mb-1">{data.hero.location}</p>
          <h1 className="text-2xl font-bold text-white font-sans leading-tight">
            {data.hero.firstName}<br />{data.hero.lastName}
          </h1>
          <p className="text-neutral-400 mt-1 font-sans text-sm">{data.hero.roles[0]}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col p-5 border border-neutral-800 rounded-xl bg-black/50 backdrop-blur-sm gap-4">
        {data.socials
          ...
          .map((social) => (
            <Link key={social.name} href={social.url} target="_blank" className="flex items-center gap-3 hover:text-white transition-colors">
              <img src={social.icon} alt={social.name} className="w-5 h-5 invert opacity-70" />
              <span>{social.mobileLabel}</span>
            </Link>
          ))}
        
        {/* Email is slightly different style, so handled separately */}
        <a href={`mailto:${data.hero.email}`} className="flex items-center gap-3 hover:text-white transition-colors">
      ...
      <div className="flex gap-3 mt-2 font-sans font-medium">
        <a href={data.hero.resumeLink} onClick={handleDownload} download className="flex flex-1 justify-center items-center gap-2 py-3 text-center border border-white text-white rounded-lg hover:bg-white hover:text-black transition-colors">
