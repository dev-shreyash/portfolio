...
export default function AboutSection() {
  ...
  const { about } = portfolioData; // Destructure the about data for cleaner code
    ...
    <main id="about" ref={container} className="w-full max-w-7xl mx-auto px-4 pt-10 pb-0 bg-transparent text-gray-300 font-mono text-lg relative z-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
        ...
        <div className="about-card md:col-span-2 p-6 border border-neutral-800 rounded-2xl bg-black/50 backdrop-blur-sm hover:border-neutral-600 transition-colors flex flex-col justify-center">
          <h2 className="text-white font-bold mb-3 font-sans text-xl">{about.title}</h2>
          <p className="text-sm leading-relaxed text-neutral-400">
            {about.description}
            <span className="inline-block bg-neutral-800 text-white px-2 py-0.5 rounded ml-2 text-xs font-sans mt-2 md:mt-0">{about.badge}</span>
          </p>
        </div>
        ...
        <div className="about-card md:col-span-1 p-6 border border-neutral-800 rounded-2xl bg-black/50 backdrop-blur-sm hover:border-neutral-600 transition-colors">
          <h2 className="text-white font-bold mb-4 font-sans text-xl">{about.educationTitle}</h2>
          <div className="space-y-4">
            {about.education.map((edu, index) => (
              <div key={index} className="pt-4 border-t border-neutral-700 flex flex-col">
                <p className="text-white text-sm font-bold font-sans">{edu.degree}</p>
                <p className="text-xs text-neutral-500">{edu.details}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="about-card md:col-span-2 p-6 border border-neutral-800 rounded-2xl bg-black/50 backdrop-blur-sm hover:border-neutral-600 transition-colors flex flex-col justify-center">
          <h2 className="text-white font-bold mb-3 font-sans text-xl">{about.beyondTitle}</h2>
          <p className="text-sm leading-relaxed text-neutral-400">
        ...
        <div className="about-card md:col-span-1 p-6 border border-neutral-800 rounded-2xl bg-black/50 backdrop-blur-sm hover:border-neutral-600 transition-colors flex flex-col justify-center">
          <h2 className="text-white font-bold mb-5 font-sans text-xl text-center md:text-left">{about.techStackTitle}</h2>
